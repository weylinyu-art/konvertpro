...
import { TIP_MODULES, getAllDetailedTipArticles } from "@/lib/conversion-tips-data";
...
export function getCoreUrls() {
  const urls = [
    ...
    ...TIP_MODULES.map((m) => `${BASE_URL}/conversion-tips/module/${m.key}`),
    ...getAllDetailedTipArticles().map((a) => `${BASE_URL}/conversion-tips/${a.slug}`),
    `${BASE_URL}/currency`,
    `${BASE_URL}/compare`,
